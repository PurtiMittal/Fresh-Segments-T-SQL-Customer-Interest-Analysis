-- Interest Analysis
 ...
 -- To find this, we count distinct month_year apperances per interest and compare against the total number of distinct months in the dataset usng a subquery.
...
-- Result: Cumulative percentage crosses 90% at total_months = 6
-- Interests present for 6 or more months account for over 90% of all records.
...
-- Result - Absolutely. Consider two interests from the dataset:
...
-- We are only removing 400 records which is 3.06% of the dataset.
-- This is a very small price for a meaningfully cleaner analysis.
...
-- Result - The filtered dataset retains a healthy number of interests across all months, confirming the 6-month threshold strikes the right balance between data quality and analytical coverage.